...
#ifdef CONFIG_ZRAM
static int inactive_anon_reclaim_prefer(struct lruvec *lruvec)
{
	unsigned long inactive_anon;
	unsigned long inactive_file;
	unsigned long ratio = current_is_kswapd()?4:8;
	inactive_anon = get_lru_size(lruvec, LRU_INACTIVE_ANON);
	inactive_file = get_lru_size(lruvec, LRU_INACTIVE_FILE);

	return inactive_anon > ratio*inactive_file;
}
#else
static int inactive_anon_reclaim_prefer(struct lruvec *lruvec)
{
			   ...
			   unsigned long *nr)
{
	 ...
	 */
	if (!inactive_file_is_low(lruvec) && !inactive_anon_reclaim_prefer(lruvec)) {
		scan_balance = SCAN_FILE;
		goto out;
	}
