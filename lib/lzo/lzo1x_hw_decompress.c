...
int _lzo1x_decompress_safe_hw(const unsigned char *src , size_t src_len , unsigned char *dst , size_t *dst_len)
{
...
{
	return ZipDec_decompress(src,src_len,dst,dst_len);
}
...
static int __init ZipDec_Init(void)
{
	...
	ZipDec_decompress = get_zip_type() ?  lzo1x_decompress_safe : _lzo1x_decompress_safe_hw;
	//ZipDec_Set_Timeout(zipdec_param.timeout);
	return 0;
}
