...
bool wifi_cali_file_check(const char *path)
{
	...
	unsigned char *pBuf = NULL;
	unsigned short len = 0;
	unsigned short len_1 = 0;
    loff_t pos;
	mm_segment_t fs = get_fs();

	set_fs(KERNEL_DS);
    printk("wifi_cali_file_check enter +++++++++++++++++++++++\n");
    filp = filp_open(path, O_RDONLY, S_IRUSR);
    if (IS_ERR(filp))
    {
        printk("%s,Unable to load '%s' err=%ld.\n", __func__, path, PTR_ERR(filp));
    ...
    {
	    len_1 = i_size_read(file_inode(filp));
	    printk("cali_file_check:path[%s] len[%d]\n",path,len_1);
	    if (!( len > 0 && len < 62 * 1024 ))
	    {
        ...
        printk( "%s,connectivity_calibration.ini check oK. 1850 \n", __func__);        goto file_check_done;
    }
...
void ittiam_nvm_init(void )
{
    ...
    || (defined CONFIG_MACH_SPX35EC) || (defined CONFIG_MACH_SP8830GA) || (defined CONFIG_MACH_SP8835EB) \
	|| (defined CONFIG_MACH_KANAS_W) || (defined CONFIG_MACH_KANAS_TD) \
   || (defined CONFIG_MACH_SP5735C1EA) || (defined CONFIG_MACH_SP5735EA) \
   || (defined CONFIG_MACH_SP6815EA) || (defined CONFIG_MACH_SP6815GA)
