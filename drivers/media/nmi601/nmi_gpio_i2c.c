...
#if 0
#define mt_set_gpio_out(pin,level)  gpio_direction_output(pin,level) 
#define mt_get_gpio_in(value)      gpio_get_value(value)
#define mt_set_gpio_dir(pin,is_input)          __gpio_set_direction(pin,is_input)
#else


#define NMI_SDA_PIN	184
#define NMI_SCL_PIN	185


#define set_sda_output() gpio_direction_output(NMI_SDA_PIN, 0)
	//gpio_direction_output(NMI_SDA_PIN, 0);\
			//}while(0)
...
#define set_i2c_sda_PIN  			mt_set_gpio_out(NMI_SDA_PIN,1)
#define clr_i2c_sda_PIN    		    	mt_set_gpio_out(NMI_SDA_PIN,0)
#else
#define set_i2c_scl_PIN  			mt_set_gpio_out(NMI_SCL_PIN,1)	
#define clr_i2c_scl_PIN   			mt_set_gpio_out(NMI_SCL_PIN,0)
...
#define get_i2c_sda_PIN   			mt_get_gpio_in(NMI_SDA_PIN)
#define get_i2c_scl_PIN   		    mt_get_gpio_in(NMI_SCL_PIN)

#else
#ifdef _DRIVE_
#define set_i2c_scl_PIN  gpio_set_value(NMI_SCL_PIN,1)
		//	}while(0)	
#define clr_i2c_scl_PIN  	gpio_set_value(NMI_SCL_PIN,0)
	//		}while(0)
#define set_i2c_sda_PIN  gpio_set_value(NMI_SDA_PIN,1)
		//	}while(0)
#define clr_i2c_sda_PIN  gpio_set_value(NMI_SDA_PIN,0)
		//	}while(0)
 #else
#define set_i2c_scl_PIN gpio_set_value(NMI_SCL_PIN,1)
		//	}while(0)	
#define clr_i2c_scl_PIN  gpio_set_value(NMI_SCL_PIN,0)
//			}while(0)
#define set_i2c_sda_PIN  gpio_set_value(NMI_SDA_PIN,0)
		//	}while(0)
#define clr_i2c_sda_PIN  gpio_set_value(NMI_SDA_PIN,1)
		//	}while(0)
#endif
...
{
	int ret = -1;
	ret = gpio_request(NMI_SDA_PIN,  "i2c_sda");
	if(ret) {
		printk("NMI_SDA_PIN gpio_request failed!\n");
	}
//        gpio_direction_output(NMI_SDA_PIN, 0);
	
	ret = gpio_request(NMI_SCL_PIN,  "i2c_scl");
	if(ret) {
		printk("NMI_SCL_PIN gpio_request failed!\n");
	}
}
