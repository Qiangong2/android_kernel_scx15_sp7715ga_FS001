...
static struct wake_lock sensor_wakelock;
...
LOCAL int _sensor_k_ccirck(BOOLEAN is_ccirck0)
{
	SENSOR_PRINT_HIGH("SENSOR: is_ccirck0 %d\n", is_ccirck0);
	if(is_ccirck0) {
        sci_glb_clr(SPRD_PIN_BASE + 0x4, BIT(26) | BIT(25));
	}
	else {
        sci_glb_clr(SPRD_PIN_BASE + 0x4, BIT(26));
        sci_glb_set(SPRD_PIN_BASE + 0x4, BIT(25));
	}
				...
				unsigned long arg)
{
	...
	switch (cmd) {
	case SENSOR_IO_CCIRCK:
		{
			BOOLEAN is_ccirck0;
			ret = copy_from_user(&is_ccirck0, (BOOLEAN *) arg, sizeof(BOOLEAN));

			if (0 == ret)
				ret = _sensor_k_ccirck(is_ccirck0);
		}
		break;
...
gpio_err_exit:
	...
	wake_lock_init(&sensor_wakelock, WAKE_LOCK_SUSPEND,"mm_wakelock_sensor");
	return ret;
}
