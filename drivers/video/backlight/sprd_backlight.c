 ...
 */
	...
	#if !defined(CONFIG_ZYT_SUPPORT)	// alex.shi Defaultly,if our company use whiteled,it must be parallel mode
...
#if(defined(CONFIG_MACH_SP7715EA)||defined(CONFIG_MACH_SP7715EATRISIM)||defined(CONFIG_MACH_SP7715EGOOD))
#if((!defined(CONFIG_MACH_SP7715EA_7671))&&(!defined(CONFIG_BACKLIGHT_ZYT_PARALLEL_MODE)))
...
#if defined(CONFIG_MACH_SP7715EA_7671) || defined(CONFIG_MACH_SP7715GA_7672) || defined(CONFIG_MACH_SP7715GA_7673) || defined(CONFIG_MACH_SP7715GA_5701) || defined(CONFIG_BACKLIGHT_ZYT_PARALLEL_MODE)
...
//	sci_adi_set(ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_PD);
	...
	#else
	/*
sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL1, 0x4000);

sci_adi_raw_write(PD_PWM_BASE+PWM_CNT, 0x100);
sci_adi_raw_write(PD_PWM_BASE+PWM_TONE_DIV, 0x190);
sci_adi_raw_write(PD_PWM_BASE+PWM_PRESCALE, 0x10d);
sci_adi_raw_write(PD_PWM_BASE+PWM_PAT_LOW, 0xffff);
sci_adi_raw_write(PD_PWM_BASE+PWM_PAT_HIG, 0xffff);

sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL0, 0x103);
sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL1, 0xc000);
*/
...
sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL0, 0x103);
sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL1, 0xc000);
			//sci_adi_raw_write(0x400388d4, 0x7e); //0x2 ~ 0x7e brightness 
			//udelay(100); 
		//	PRINT_INFO("zyt----sprd backlight power on -kernel-(SPX15 use WHITE_LED backlight control)\n"); 
		    }while((--count > 0) && !(sci_adi_read(PD_PWM_BASE+PWM_PAT_LOW) & 0x0000ffff));
}
	//		PRINT_INFO("zyt----sprd backlight power on -kernel-(SPX15 use WHITE_LED backlight control)\n"); 

		//	PRINT_INFO("(sci_adi_read(0x4003802c)=%x\n", sci_adi_read(PD_PWM_BASE+PWM_PAT_LOW));
...
static int sprd_bl_whiteled_update_status(struct backlight_device *bldev)
{
	...
	if ((bldev->props.state & (BL_CORE_SUSPENDED | BL_CORE_FBBLANK)) ||
		...
		if (sprdbl.pwm_mode == dim_pwm) {
			...
			#if defined(CONFIG_MACH_SP7715EA_7671) || defined(CONFIG_MACH_SP7715GA_5701) || defined(CONFIG_MACH_SP7715GA_7672) || defined(CONFIG_MACH_SP7715GA_7673) || defined(CONFIG_BACKLIGHT_ZYT_PARALLEL_MODE)
			sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL0, 1);
			#else
			sci_adi_set(ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_PD);
			#endif
		}
	} else {
#if defined(CONFIG_LCD_HVGA) && !(defined(CONFIG_MACH_SP6815GA_7271)&&(3==CONFIG_ZYT_CUST)) && (67 != CONFIG_ZYT_CUST)
		// reduce backlight 20141110
		if (bldev->props.brightness>1)
			bldev->props.brightness = bldev->props.brightness * 17 / 20;
		// reduce backlight 20141110
#endif
		bl_brightness = bldev->props.brightness & PWM_MOD_MAX;
		...
		if (sprdbl.pwm_mode == dim_pwm) {
			...
			#if defined(CONFIG_MACH_SP7715EA_7671) || defined(CONFIG_MACH_SP7715GA_7672) || defined(CONFIG_MACH_SP7715GA_7673) || defined(CONFIG_MACH_SP7715GA_5701) || defined(CONFIG_BACKLIGHT_ZYT_PARALLEL_MODE)
			/*dimming pwm config*/
			//sci_adi_raw_write(DIMMING_PWD_BASE + PWM_PRESCALE, PWM_PRESCALE);
			sci_adi_raw_write(DIMMING_PWD_BASE + PWM_CNT,
					(pwm_level << 8) | PWM_MOD_MAX);
			sci_adi_raw_write(DIMMING_PWD_BASE + PWM_PRESCALE,
			...
			#if defined(CONFIG_MACH_SP7715EA_7671) || defined(CONFIG_MACH_SP7715GA_7672) || defined(CONFIG_MACH_SP7715GA_7673) || defined(CONFIG_MACH_SP7715GA_5701) || defined(CONFIG_BACKLIGHT_ZYT_PARALLEL_MODE)
			sci_adi_raw_write(ANA_REG_GLB_WHTLED_CTRL1, 0);
			sci_adi_raw_write(PD_PWM_BASE + PWM_CNT,
					(pwm_level << 8) | PWM_MOD_MAX);

			sci_adi_raw_write(PD_PWM_BASE + PWM_PRESCALE,
					PWM_SCALE | PWM_ENABLE);
			#else
...
{
#if (CONFIG_ZYT_CUST == 67) && (CONFIG_ZYT_PRJ == 7675)
	msleep(150);
#else
	msleep(50);
#endif
