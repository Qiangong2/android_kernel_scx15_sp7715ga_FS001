...
#ifdef CONFIG_FB_ESD_SUPPORT
static int32_t sprdfb_dispc_refresh_esd (struct sprdfb_device *dev)
{
	uint32_t reg_val = 0;
	...
	if(SPRDFB_PANEL_IF_DPI != dev->panel_if_type){
		...
		dispc_sync(dev);
//		dispc_ctx.vsync_done = 0;
	printk(KERN_INFO "sprdfb:*888888888[%s]\n",__FUNCTION__);
	}
...
//	dispc_ctx.dev = dev;
	...
	printk(KERN_INFO "sprdfb:***********2************[%s]\n",__FUNCTION__);

#ifdef CONFIG_FB_LCD_OVERLAY_SUPPORT
	printk(KERN_INFO "sprdfb:************3***********[%s]\n",__FUNCTION__);
...
ERROR_REFRESH:
	...
	printk("DISPC_CTRL: 0x%x\n", dispc_read(DISPC_CTRL));
	printk("DISPC_STS: 0x%x\n", dispc_read(DISPC_STS));
	printk("DISPC_DBI_CTRL: 0x%x\n", dispc_read(DISPC_DBI_CTRL));
	printk("DISPC_DBI_TIMING0: 0x%x\n", dispc_read(DISPC_DBI_TIMING0));
	printk("DISPC_DBI_TIMING1: 0x%x\n", dispc_read(DISPC_DBI_TIMING1));
	printk("DISPC_DBI_RDATA: 0x%x\n", dispc_read(DISPC_DBI_RDATA));
	printk("DISPC_DBI_CMD: 0x%x\n", dispc_read(DISPC_DBI_CMD));
	printk("DISPC_DBI_DATA: 0x%x\n", dispc_read(DISPC_DBI_DATA));
	printk("DISPC_DBI_QUEUE: 0x%x\n", dispc_read(DISPC_DBI_QUEUE));
	printk("DISPC_SIZE_XY: 0x%x\n", dispc_read(DISPC_SIZE_XY));

	printk("DISPC_BG_COLOR: 0x%x\n", dispc_read(DISPC_BG_COLOR));

	printk("DISPC_INT_EN: 0x%x\n", dispc_read(DISPC_INT_EN));

	printk("DISPC_OSD_CTRL: 0x%x\n", dispc_read(DISPC_OSD_CTRL));
	printk("DISPC_OSD_BASE_ADDR: 0x%x\n", dispc_read(DISPC_OSD_BASE_ADDR));
	printk("DISPC_OSD_SIZE_XY: 0x%x\n", dispc_read(DISPC_OSD_SIZE_XY));
	printk("DISPC_OSD_PITCH: 0x%x\n", dispc_read(DISPC_OSD_PITCH));
	printk("DISPC_OSD_DISP_XY: 0x%x\n", dispc_read(DISPC_OSD_DISP_XY));
	printk("DISPC_OSD_ALPHA	: 0x%x\n", dispc_read(DISPC_OSD_ALPHA));
	return 0;
}
#endif
...
//#if 0//huanggq
			...
			sprdfb_panel_resume(dev, true);
#else
                        sprdfb_dispc_early_init(dev);//huanggq, not call, will black screen
                        sprdfb_panel_resume(dev, false);//huanggq, not reset and init
                        sprdfb_dispc_init(dev);//huanggq, not call, will black screen
#endif
		}
...
static int32_t sprdfb_dispc_check_esd_dbi(struct sprdfb_device *dev)
{
	uint32_t ret = 0;
...
//	sprdfb_panel_suspend(dev);
//	printk("sprdfb_panel_suspend******************\n");
//	sprdfb_panel_resume(dev,true);
//	printk("sprdfb_panel_resume*********************\n");
//	sprdfb_dispc_init(dev);
//	dev->enable = 1;
//	if(dev->panel->is_clean_lcd){
//		sprdfb_dispc_clean_lcd(dev);
//	}
	sprdfb_dispc_refresh_esd(dev);
	printk("sprdfb_dispc_refresh********************\n");
#endif
	dispc_ctx.vsync_waiter ++;
    dispc_sync(dev);	
	ret = sprdfb_panel_ESD_check(dev);
...
static int32_t sprdfb_dispc_check_esd(struct sprdfb_device *dev)
{
	...
	if(SPRDFB_PANEL_IF_DBI == dev->panel_if_type){
		...
		sprdfb_dispc_check_esd_dbi(dev);
		//ret = -1;
					...
					u32 new_val, int howto)
{
	...
	if (howto != SPRDFB_FORCE_PCLK &&
			...
			!fb_dev->enable) {
		pr_warn("After fb_dev is resumed, dpi clk will be updated\n");
		return ret;
	}
	...
	pr_info("dpi clock is switched from %dHz to %dHz and fps is %u\n",
					fb_dev->dpi_clock, new_pclk, panel->fps);
