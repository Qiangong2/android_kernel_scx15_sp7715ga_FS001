...
typedef void (*spi_send_cmd_t)(uint32_t length,uint32_t cmd);
typedef void (*spi_send_data_t)(uint32_t length,uint32_t data);
typedef void (*spi_read_t)(uint32_t length,uint32_t *data);
#else
...
struct ops_spi{
	#if defined(CONFIG_FB_SC7715_RGB_SPI_16BIT_AND_8BIT_ZYT_SUPPORT)
	void (*spi_send_cmd)(uint32_t length,uint32_t cmd);
	void (*spi_send_data)(uint32_t length,uint32_t data);
	void (*spi_read)(uint32_t length,uint32_t *data);
	#else
