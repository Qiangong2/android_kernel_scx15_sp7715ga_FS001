...
#define LOWMEM_ADJ_NUM_MAX (7)

static uint32_t lowmem_debug_level = 1;
static short lowmem_adj[LOWMEM_ADJ_NUM_MAX] = {
	0,
	1,
	6,
	12,
...
static int lowmem_minfree[LOWMEM_ADJ_NUM_MAX] = {
	3 * 512,	/* 6MB */
	2 * 1024,	/* 8MB */
	4 * 1024,	/* 16MB */
	16 * 1024,	/* 64MB */
...
static struct task_struct * lowmem_repeatedly_selected;
...
static uint zone_wmark_ok_safe_gap = 768;
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
...
#ifdef CONFIG_ZRAM
	/*set the weight of zram score adj by 80 precent*/
	zram_score_adj = cacl_zram_score_adj()*80/100;
	if(zram_score_adj  < 0 )
	{
	...
	if(min_score_adj < zram_score_adj)
	{
		...
		if( (zram_score_adj <= OOM_ADJ_TO_OOM_SCORE_ADJ(1)) && (min_score_adj >=OOM_ADJ_TO_OOM_SCORE_ADJ(1)) )
			zram_score_adj = min_score_adj;
	}
	...
	for_each_process(tsk) {
		...
		/*skip TASK_UNINTERRUPTIBLE task*/
	...
	if (selected && (selected_oom_score_adj || is_need_lmk_kill)) {
			...
			lowmem_repeatedly_selected = selected;
...
static int lowmem_adj_array_get(char *buffer, const struct kernel_param *kp)
{
	...
	{
		char temp[2] = {0};
		int oom_adj[LOWMEM_ADJ_NUM_MAX] = {0};
		int t = 0;
		for(t = 0; t < ARRAY_SIZE(lowmem_adj); t++){
			memset(temp,0,sizeof(temp));
			oom_adj[t] = lowmem_oom_score_adj_to_oom_adj(lowmem_adj[t]);
			if ( t == ARRAY_SIZE(lowmem_adj) - 1 )
				sprintf(temp, "%d", oom_adj[t]);
			else
				sprintf(temp, "%d,", oom_adj[t]);
			buffer = strncat(buffer, temp, strlen(temp));
		}
