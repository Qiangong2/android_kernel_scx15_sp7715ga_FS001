...
#define ZYT_PERIPHERALS_SENSOR_PARA_MAIN	"/data/local/media/zyt_peripherals.sensorMain"
#define ZYT_PERIPHERALS_SENSOR_PARA_SUB	"/data/local/media/zyt_peripherals.sensorSub"
...
static ssize_t __zyt_peripherals_get_peripherals_info(struct zyt_peripherals_dev* dev, char* buf) {
	...
	if(0 == b_has_got_peripherals_info)
	{
		...
		//for LCD name
		len = __zyt_peripherals_get_name_by_id(NULL, dev->peripherals_info+ascending_len, TYPE_LCD);
		ascending_len += len;
		dev->peripherals_info[ascending_len++] = ';';

		//for main CAMERA name
		len = __zyt_peripherals_get_name_by_id(NULL, dev->peripherals_info+ascending_len, TYPE_CAMERA_MAIN);
		ascending_len += len;
		dev->peripherals_info[ascending_len++] = ';';
		//for sub CAMERA name
		len = __zyt_peripherals_get_name_by_id(NULL, dev->peripherals_info+ascending_len, TYPE_CAMERA_SUB);
		ascending_len += len;
		dev->peripherals_info[ascending_len++] = ';';

		//for GSENSOR name
		len = __zyt_peripherals_get_name_by_id((void*)CDC_Gsensor_Device_Id(0), dev->peripherals_info+ascending_len, TYPE_GSENSOR);
		ascending_len += len;
		dev->peripherals_info[ascending_len++] = ';';

		//for PLSENSOR name
		len = __zyt_peripherals_get_name_by_id((void*)CDC_Plsensor_Device_Id(0), dev->peripherals_info+ascending_len, TYPE_PLSENSOR);
		ascending_len += len;
		dev->peripherals_info[ascending_len++] = ';';
