...
#define IN_IQ_SPECIAL_DDR_BASE 0x30000000
#define IN_IQ_SPECIAL_DDR_OFFSET 0x25c
#define IN_IQ_SPECIAL_DDR_VALUE 0xf00000ff
unsigned long iq_special_ddr_base;
static void init_special_ddr_reg(void)
{
	iq_special_ddr_base = ioremap(IN_IQ_SPECIAL_DDR_BASE, SZ_4K);
	if (!iq_special_ddr_base)
		panic("can't create iq_special_ddr_base !");
	__raw_writel(IN_IQ_SPECIAL_DDR_VALUE, iq_special_ddr_base + IN_IQ_SPECIAL_DDR_OFFSET);
}
