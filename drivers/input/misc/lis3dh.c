...
#define GSENSOR_DBG(format, ...)	\
		printk(KERN_INFO "LIS3D " format, ## __VA_ARGS__)
#else
#define GSENSOR_DBG(format, ...)
#endif
					    ...
					    int *xyz)
{
    ...
    acc_xyz_data[0] = xyz[0];
    acc_xyz_data[1] = xyz[1];
    acc_xyz_data[2] = xyz[2];
...
static int lis3dh_acc_enable(struct lis3dh_acc_data *acc)
{
	...
	GSENSOR_DBG("sprd-gsensor: -- %s -- success!\n",__func__);
	return 0;
}
				    ...
				    const char *buf, size_t count)
{
	...
	if(enable)
	{  
		ret = lis3dh_acc_enable(acc);
		if(!ret){
			GSENSOR_DBG("lis3dh enable setting active \n");
		}
	}
	else
	{
		ret = lis3dh_acc_disable(acc);
		if(!ret){
			GSENSOR_DBG("lis3dh enable setting inactive \n");
		}
	}
...
static ssize_t lis3dh_chip_id_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", WHOAMI_LIS3DH_ACC);
}

static ssize_t lis3dh_xyz_data_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
...
{
	CDC_Gsensor_Device_Id(WHOAMI_LIS3DH_ACC);
...
#ifdef CONFIG_ZYT_GSENSOR_COMPATIBLE
    err = sysfs_create_group(&(acc->input_dev->dev.kobj), &lis3dh_attr_group);
	if (err) {
...
err_free_irq2:
	if (acc->irq2) {
		cancel_work_sync(&acc->irq2_work);
...
err_free_irq1:
	if (acc->irq1) {
		cancel_work_sync(&acc->irq1_work);
...
#ifdef CONFIG_ZYT_GSENSOR_COMPATIBLE
	sysfs_remove_group(&(acc->input_dev->dev.kobj), &lis3dh_attr_group);
#endif
#if 0	//def CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&acc->early_suspend);
#endif
