 ...
 *
 * 思路概述  
 *	als和ps均采用定时器中断来定时执行采样.als是stk_als_poll_work_func,ps是stk_ps_poll_work_func;
 * 	采样间隔分别为 ps_data->als_poll_delay 和 ps_data->ps_poll_delay.这两个时间前者默认为110ms并
 *  可以通过上层修改,后者默认为60ms,且上层修改不了
 *
 */
...
// Alex.shi SPREADTRUM_PLATFORM 只在展讯MocorDroid2.3.5平台上才能打开
...
#define STK_SINGLE_INPUT_DEVICE
...
#ifdef CONFIG_ZYT_PLSENSOR_COMPATIBLE
#define PS_NAME			"stk3x1x-pls"
#ifndef STK_SINGLE_INPUT_DEVICE
#define STK_SINGLE_INPUT_DEVICE
#endif
#else
...
#if ((STK_INT_PS_MODE == 0x03) || (STK_INT_PS_MODE	== 0x02))	
	err = request_any_context_irq(irq, stk_oss_irq_handler, IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, STK3310_DEVICE_NAME, ps_data);
#else	
	err = request_any_context_irq(irq, stk_oss_irq_handler, IRQF_TRIGGER_LOW, STK3310_DEVICE_NAME, ps_data);
#endif	
	if (err < 0) 
	{
...
#if 0	//def CONFIG_HAS_EARLYSUSPEND // Alex.shi 否则会导致通话界面死机
...
#ifdef CONFIG_ZYT_PLSENSOR_COMPATIBLE
static struct device_attribute cdc_als_enable_attribute = __ATTR(als_enable,0664,stk_als_enable_show,stk_als_enable_store);
static struct device_attribute cdc_als_poll_delay_attribute = __ATTR(als_delay,0664,stk_als_delay_show,stk_als_delay_store);
static struct device_attribute cdc_ps_enable_attribute = __ATTR(ps_enable,0664,stk_ps_enable_show,stk_ps_enable_store);
...
static struct device_attribute cdc_chip_id_attribute = __ATTR(chip_id,0664,chip_id_show,NULL);
                        ...
                        const struct i2c_device_id *id)
{
	...
	// Alex.shi 做了兼容后需要在这里改回实际addr  
	client->addr = STK3310_ADDRESS;
	printk("STK3x1x I2C Address(final): 0x%02x", client->addr);
#endif
...
#ifdef STK_SINGLE_INPUT_DEVICE
	ps_data->als_input_dev = ps_data->ps_input_dev;
//	printk(KERN_ERR "%s: als_input_dev=0x%x, ps_input_dev=0x%x\n", __func__, ps_data->als_input_dev, ps_data->ps_input_dev);		
#else
