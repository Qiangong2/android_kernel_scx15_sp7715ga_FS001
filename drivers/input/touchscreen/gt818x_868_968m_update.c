...
*1. INT port output low£¬delay 5ms
*2. RESET port output low 100ms£¬change to input
*3. I2C access GUITAR
*4. delay 100ms and read register 0xff(step3¡¢step4 retry 80 times£¬until success)
*5. Oxff == 0x55 success
*/
    ...
    else
    {
        ...
        for(retry=0; retry<20; retry++)
        {
            ...
            if (ret > 0)
            {
                ...
                if (ret > 0)
                {
                    GTP_DEBUG("The value of 0x00ff is 0x%02x", buf[2]);
                    if(buf[2] == 0x55)
                    {
                        GTP_DEBUG("GT868 Wait unlock nvram successfully!");
                        break;
                    }
                }  
            }
        ...
        if (i >= 5)
        {
            GTP_ERROR("GT868 Wait unlock nvram failed.");
            return fail;
        }
    }
