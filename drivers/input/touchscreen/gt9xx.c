...
#include "gt9xx.h"
#if 1//def CONFIG_MACH_SP7715GA_5701
...
// Alex.shi ???GT9xx?GT8xx????????????  
#define GT_COPATIBLE_9XX_8XX
...
static const char *goodix_ts_name = "SPRD_WW_CTP";//goodix-ts
...
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_MENU) ":40:880:50:30"
		":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE) ":120:880:50:30"
		":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":200:880:50:30"
		":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":280:880:50:30"
		"\n");
}
...
//printk("finger=========================%d\n", finger);
    ...
    if (gtp_proximity_start == 1)
    {
		if (point_data[GTP_ADDR_LENGTH] & 0x20)//if (point_data[GTP_ADDR_LENGTH] & 0x60)
		{
			...
			PS_DBG("proximity_data-ABS_DISTANCE-==00000000000000000\n");
			input_sync(ts->input_dev);
            goto exit_work_func;
		}
		else
		{
			...
			PS_DBG("proximity_data-ABS_DISTANCE-==11111111111111111\n");

		}
   		PS_DBG("proximity_data[2] = 0x%x\n", point_data[GTP_ADDR_LENGTH]);
   }   
#endif	
...
//printk("touch_num==========%d\n", touch_num);
    ...
    if(key_value || pre_key)
    {
		...
		if ((1==key_value) || (2==key_value) || (4==key_value) || (8==key_value))
		{
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 880);
			input_mt_sync(ts->input_dev);
			input_sync(ts->input_dev);
...
static int remove_gt9xx_chip(void)
{
	ctp_lock_mutex();
	tp_device_id(0xFFFF);
	ctp_unlock_mutex();
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    u16 version_info;
#if 1//def CONFIG_MACH_SP7715GA_5701
	struct gt9xx_ts_platform_data *pdata = client->dev.platform_data;
...
#ifdef GT_COPATIBLE_9XX_8XX	// Alex.shi ??????????????addr  
	client->addr = GTP_TS_ADDR;
	goodix_ts_setup.i2c_address = GTP_TS_ADDR;
	GTP_INFO("GTP I2C Address(final): 0x%02x", client->addr);
#endif
...
#endif
{
	if(tp_device_id(0)!=0)
	{
		GTP_DEBUG("CTP(0x%x)Exist!", tp_device_id(0));
		return -ENODEV;
	}
...
static int sprd_add_i2c_device(struct sprd_i2c_setup_data *i2c_set_data, struct i2c_driver *driver)
{
	...
	if (!adapter) {
		GTP_DEBUG("%s: can't get i2c adapter %d\n",
	...
	if (!client) {
		GTP_DEBUG("%s:  can't add i2c device at 0x%x\n",
	...
	if (ret != 0) {
		GTP_DEBUG("%s: can't add i2c driver\n", __func__);
		err = -ENODEV;
		goto err_driver;
	}	
...
{	
	...
	// Alex.shi ?GT???device????board.c????????????,????,?????  
	goodix_ts_setup.i2c_bus = 1;
#ifdef GT_COPATIBLE_9XX_8XX	// Alex.shi ????gt868?gt9xx,?????????  
	goodix_ts_setup.i2c_address = 0x14;
#else
	goodix_ts_setup.i2c_address = GTP_TS_ADDR;
#endif
