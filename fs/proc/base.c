			    ...
			    loff_t *ppos)
{
	...
	if (lock_task_sighand(task, &flags)) {
		if (task->signal->oom_score_adj == OOM_SCORE_ADJ_MAX)
			...
			oom_adj = (task->signal->oom_score_adj * -OOM_DISABLE + (OOM_SCORE_ADJ_MAX - 1)) /
				  OOM_SCORE_ADJ_MAX;
		unlock_task_sighand(task, &flags);
	}
	put_task_struct(task);
			     ...
			     size_t count, loff_t *ppos)
{
	...
	if(!strncmp("commands.monkey", task->comm, 15)) oom_adj = OOM_DISABLE;
	/*
	...
	//if(!strncmp("commands.monkey", task->comm, 15)) oom_adj = -16;
