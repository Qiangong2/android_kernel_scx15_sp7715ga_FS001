...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
...
typedef struct blk_requestinfo {
	unsigned long dbg_count;
	struct dbg_sdcard_t sprd_dbg_sdcard[256];
}blk_requestinfo;
#endif
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
	struct blk_requestinfo blk_req_info;
#endif
}cpu_recorder;
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
static DEFINE_SPINLOCK(blk_request_lock);
#endif
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
extern struct dbg_sdcard_t  sprd_dbg_sdcard[256];
extern int sprd_dbg_request_cnt;
#endif
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
        spin_lock(&blk_request_lock);
	recorder[pos].blk_req_info.dbg_count = sprd_dbg_request_cnt;
	if (sprd_dbg_request_cnt > 0) {
		memcpy(recorder[pos].blk_req_info.sprd_dbg_sdcard, sprd_dbg_sdcard, sizeof(struct dbg_sdcard_t) * sprd_dbg_request_cnt);
		sprd_dbg_request_cnt = 0;
		memset(sprd_dbg_sdcard, 0, sizeof(struct dbg_sdcard_t) * 256);
	}
        spin_unlock(&blk_request_lock);
#endif
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
	seq_printf(p, "IO Request info:\n");
	seq_printf(p, "%16s %6s %6s %6s %6s\n", "COMM","PID","COUNT","TEMP","REQ SIZE");
	for(i = 0; i < (record->blk_req_info).dbg_count; i++){
		seq_printf(p, "%16s %6d %6d %6d %6d\n", (record->blk_req_info).sprd_dbg_sdcard[i].comm,	\
							(record->blk_req_info).sprd_dbg_sdcard[i].pid,	\
							(record->blk_req_info).sprd_dbg_sdcard[i].cnt,	\
							(record->blk_req_info).sprd_dbg_sdcard[i].request_size, i);
	}
#endif
}
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
static void anr_dumpstack(void)
{
	struct task_struct *tsk = NULL;
	unsigned char *kswapd = "kswapd0";
	unsigned char *mmcqd = "mmcqd";

	printk("Dump stack for ANR debug: \n");
	rcu_read_lock();
	for_each_process(tsk) {
		if( (!strncmp(tsk->comm, kswapd,7)) ||
		    (!strncmp(tsk->comm, mmcqd,5)) ){
			printk("PID:%d Comm:%s: \n",tsk->pid, tsk->comm);
			show_stack(tsk, NULL);
                }else if ( (!tsk->signal->oom_score_adj)&&
...
#ifdef CONFIG_SPRD_ANR_IO_DEBUG
	/*show meminfo and kswapd&mmcqd backstrace */
